...
class NVGSpecialRenderer(vertexConsumers: MultiBufferSource.BufferSource) : PictureInPictureRenderer<NVGSpecialRenderer.NVGRenderState>(vertexConsumers) {
    ...
    override fun renderToTexture(state: NVGRenderState, poseStack: PoseStack) {
        ...
        val (fbWidth, fbHeight) = colorTex.let { it.getWidth(0) to it.getHeight(0) }
        ...
        val guiScale = OmniResolution.scaleFactor
        val guiWidth = fbWidth / guiScale
        val guiHeight = fbHeight / guiScale

        Vexel.renderer.beginFrame(guiWidth.toFloat(), guiHeight.toFloat())
        state.renderContent()
