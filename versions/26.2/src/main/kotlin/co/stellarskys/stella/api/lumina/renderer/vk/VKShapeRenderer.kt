...
internal object VKShapeRenderer {
    ...
    fun render(cmd: VkCommandBuffer, shapes: List<Lumina.QueuedShape>, vw: Int, vh: Int) {
        ...
        var sc = shapes.firstOrNull()?.scissor; var rs = 0
        for (s in shapes) {
            when (s.stencilOp) {
                1 -> {
                    if (vCount > rs) runs.add(DrawRun(sc, rs, vCount - rs))
                    sc = s.scissor; rs = vCount; filledRect(s)
                    if (vCount > rs) runs.add(DrawRun(sc, rs, vCount - rs, stencilOp = 1)); rs = vCount
                }
                2 -> { if (vCount > rs) runs.add(DrawRun(sc, rs, vCount - rs)); runs.add(DrawRun(null, 0, 0, stencilOp = 2)); rs = vCount }
                else -> {
        ...
        MemoryStack.stackPush().use { stack ->
            ...
            vkCmdPushConstants(cmd, VKPipelineManager.shapePipelineLayout, VK_SHADER_STAGE_VERTEX_BIT or VK_SHADER_STAGE_FRAGMENT_BIT, 0, projBuf)
            val viewport = VkViewport.calloc(1, stack)
            viewport[0].x(0f).y(0f).width(vw.toFloat()).height(vh.toFloat()).minDepth(0f).maxDepth(1f)
            vkCmdSetViewport(cmd, 0, viewport)
        }

        var pipeline = VKPipelineManager.shapePipeline
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline)
        for (r in runs) {
            when (r.stencilOp) {
                1 -> { applyScissor(cmd, r.scissor, vw, vh); vkCmdDraw(cmd, r.count, 1, r.start, 0); pipeline = VKPipelineManager.shapePipelineMasked; vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline) }
                2 -> { pipeline = VKPipelineManager.shapePipeline; vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline) }
                else -> { applyScissor(cmd, r.scissor, vw, vh); vkCmdDraw(cmd, r.count, 1, r.start, 0) }
            }
        }
        bufferOffset += vCount
    }
    ...
    private fun filledRect(s: Lumina.QueuedShape) {
        ...
        for (i in 0 until n) { val j = (i + 1) % n; emit(cp, cc, 1f); emit(tp(outline[i], s.transform), colorAt(s, outline[i].x, outline[i].y), 1f); emit(tp(outline[j], s.transform), colorAt(s, outline[j].x, outline[j].y), 1f) }
        emitFringe(outline, computeNormals(outline), s, fw, 1f)
    }

    private fun hollowRect(s: Lumina.QueuedShape) {
        val b = s.border; val fringe = fringeWidth(s.transform); val half = max(0f, b * 0.5f - fringe)
        val outer = LuminaBackend.generateOutline(s.x - half, s.y - half, s.w + half * 2, s.h + half * 2, s.tl + half, s.tr + half, s.br + half, s.bl + half)
        val inner = LuminaBackend.generateOutline(s.x + half, s.y + half, s.w - half * 2, s.h - half * 2, max(0f, s.tl - half), max(0f, s.tr - half), max(0f, s.br - half), max(0f, s.bl - half))
        val n = outer.size; if (n < 3 || inner.size != n) return
        for (i in 0 until n) {
            val j = (i + 1) % n
            val toi = tp(outer[i], s.transform); val toj = tp(outer[j], s.transform); val tii = tp(inner[i], s.transform); val tij = tp(inner[j], s.transform)
            val coi = colorAt(s, outer[i].x, outer[i].y); val coj = colorAt(s, outer[j].x, outer[j].y); val cii = colorAt(s, inner[i].x, inner[i].y); val cij = colorAt(s, inner[j].x, inner[j].y)
            emit(toi, coi, 1f); emit(tii, cii, 1f); emit(toj, coj, 1f); emit(toj, coj, 1f); emit(tii, cii, 1f); emit(tij, cij, 1f)
        }
        emitFringe(outer, computeNormals(outer), s, fringe, 1f); emitFringe(inner, computeNormals(inner), s, fringe, -1f)
    }
    ...
    private fun emitFringe(outline: List<Vector2f>, normals: List<Vector2f>, s: Lumina.QueuedShape, fringe: Float, sign: Float) {
        for (i in 0 until outline.size) {
            val j = (i + 1) % outline.size; val pi = outline[i]; val pj = outline[j]; val ni = normals[i]; val nj = normals[j]
            ...
            emit(ti, ci, 1f); emit(tio, ci, 0f); emit(tj, cj, 1f); emit(tj, cj, 1f); emit(tio, ci, 0f); emit(tjo, cj, 0f)
        }
    }

    private fun computeNormals(outline: List<Vector2f>) = List(outline.size) { i ->
        val n = outline.size; val prev = outline[(i - 1 + n) % n]; val curr = outline[i]; val next = outline[(i + 1) % n]
        ...
        val len = sqrt(nx * nx + ny * ny); if (len > 1e-6f) { nx /= len; ny /= len }
        Vector2f(nx, ny)
    }

    private fun emit(p: Vector2f, c: FloatArray, cov: Float) { if (vCount >= MAX_VERTS) return; buf.put(p.x).put(p.y).put(c[0]).put(c[1]).put(c[2]).put(c[3]).put(cov); vCount++ }
    ...
    private fun colorAt(s: Lumina.QueuedShape, lx: Float, ly: Float): FloatArray {
        val t = when (s.gradType) {
            ...
            3 -> { val tx = if (s.w > 0f) (lx - s.x) / s.w else 0f; val ty = if (s.h > 0f) (ly - s.y) / s.h else 0f; ((tx + ty) * 0.5f).coerceIn(0f, 1f) }
