...
internal object VKTextureRenderer {
    ...
    fun render(cmd: VkCommandBuffer, text: List<LuminaBackend.TextEntry>,
        ...
        MemoryStack.stackPush().use { stack ->
            ...
            projBuf.put(VKUtils.orthoProjection(vw, vh)).flip()
            vkCmdPushConstants(cmd, VKPipelineManager.texturePipelineLayout,
                VK_SHADER_STAGE_VERTEX_BIT or VK_SHADER_STAGE_FRAGMENT_BIT, 0, projBuf)
        ...
        for (r in runs) {
            if (r.mode != boundMode) {
                MemoryStack.stackPush().use { stack ->
                    ...
                    vkCmdPushConstants(cmd, VKPipelineManager.texturePipelineLayout,
                        VK_SHADER_STAGE_VERTEX_BIT or VK_SHADER_STAGE_FRAGMENT_BIT, 64, modeBuf)
                }
                boundMode = r.mode
            }
    ...
    private fun getOrCreateDescriptorSet(textureId: Int): Long {
        return descriptorSets.getOrPut(textureId) {
            val texHandle = VKBackend.getTextureHandle(textureId)
    ...
    // === TESSELLATION — copied from GLTextureRenderer ===
